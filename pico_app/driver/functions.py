...
def unit(unit: TIME_UNITS) -> float:
    # Each factor converts the given time unit to seconds.
    factors = [1e-15, 1e-12, 1e-9, 1e-6, 1e-3, 1]
    return factors[unit]
...
def adc2mV(bufferADC, range, maxADC):
    ...
    print(maxADC, type(maxADC))

    bufferV = [(x * float(vRange)) / maxADC for x in bufferADC]
...
def mV2adc(millivolts, range, maxADC):
    ...
    channelInputRanges = [10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000, 200000]
    vRange = channelInputRanges[range]
    adcValue = round((millivolts * maxADC)/vRange)
